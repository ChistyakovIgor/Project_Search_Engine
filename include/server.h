...
public:
     ...
     */
    std::vector<std::vector<RelativeIndex>> search(const std::vector<std::string>& queries, size_t responses_limit = 5);
    std::map<std::string, size_t> get_indexes_for_request_words(std::vector<std::string>& vec);
    void handleRequest(const std::string& request, std::vector<RelativeIndex>& , size_t responses_limit);
